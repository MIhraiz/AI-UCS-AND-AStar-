...
public class Controller implements Initializable {
	...
	/*
	 * Build the hash table by applying the dijkstra / UCS algorithm
	 */
	...
	/*
	 * Build the hash table by applying the A* (UCS with non zero heurestic) algorithm
	 */
		...
		while (!q.isEmpty()) {
			...
			for(Adjacent c: adj) {
				...
				while(itr.hasNext()) {
					...
					if (cit.getCity().equals(table.get(c.getCity().getName()))) {
						q.remove(cit);
						break;
					}
				}
				...
				// && its distance >= the distance between it and the current source city + all previous path distance
				// && are adjacent
				double newDis = c.getDistance() + temp.getCity().getDistance();
	...
	private void getDistances() throws FileNotFoundException {
		File file = new File("distance.csv");
		Scanner input = new Scanner(file);
